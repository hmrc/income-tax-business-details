...
import uk.gov.hmrc.incometaxbusinessdetails.models.updateIncomeSource.request.{UpdateIncomeSourceRequestError, UpdateIncomeSourceRequestModel}
      ...
      implicit request =>
        request.body.asJson.getOrElse(Json.obj()).validate[UpdateIncomeSourceRequestModel].fold(
          invalid => {
            ...
            UpdateIncomeSourceRequestError(INTERNAL_SERVER_ERROR, "Json validation error while parsing request")
          },
          valid => {
